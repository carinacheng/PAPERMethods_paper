...
= \frac{\bm{\Delta^T \Delta}}{N}
\end{equation}
We note that
\begin{equation}
\langle \hat{s} \rangle = s + u
\end{equation}
In this case, the quadratic estimator returns an entirely reasonable result, but, since 
$\PDeriv{\mathbf{S}}{s} = \PDeriv{\mathbf{U}}{u} =  \PDeriv{\C}{s}$, that is, {\it the covariance structure of the contaminant is identical to the signal}, the information available to the quadratic estimator to distinguish the two is degenerate, and  is only possible to achieve an unbiased estimate of $s$ by using some additional information to estimate the additive bias due to $u$ and subtract it.

Now consider a case, chosen to be very similar to the toy model in \ref{sec:},  in which the data again have an additive contaminant, now given by
\begin{equation}
...
\langle \upsilon^2 \rangle = u; \; 
 ...
 \mathbf{m}^T \mathbf{m} = N; \; {\rm and}~ 
\langle \sigma_i \upsilon \rangle = 0
\end{equation}
Here $\mathbf{m}$ represents a mode which is correlated across many data points, with amplitude given by  $\upsilon$.

We can again calculate the estimator given by \ref{eq:} explicitly by using the Sherman-Morrison identity to invert the covariance matrix.  Defining
\begin{equation}
...
\end{equation}
and the virtue of weighting by $\invC$ becomes clearer: in the limit $u \gg s$, $\xi \to 1$, and this becomes
\begin{equation}
\invC  = \frac{1}{s} \left(\I - \mathbf{m m}^T \right)
\end{equation}
where $\I - \mathbf{m m}^T$ is the projection operator, projecting out $\mathbf{m}$ from any any vector it acts on.   Proceeding to calculate the full estimator
\begin{equation}
...
The expectation is
\begin{equation}
\langle \hat{s} \rangle = \frac{ N(s + u + (\xi^2 - 2 \xi) 
[u + \langle(\bm{\sigma} \cdot \mathbf{m})^2 \rangle])} 
{N + \xi^2 - 2 \xi} 
\end{equation}
This again has a simple interpretation in the limit $\xi \to 1$, where
\begin{equation}
\langle \hat{s} \rangle = \frac{N}{N -1}  (s - \langle(\bm{\sigma} \cdot \mathbf{m})^2\rangle )
\end{equation}
Thus, in the limit the contaminant is large compared to $s$, and to the extent that $\bm \sigma \cdot \mathbf{m} \approx 0$, this yields a nearly unbiased estimator for $s$ and the inverse covariance weighting has completely removed the effect of the contaminant.
