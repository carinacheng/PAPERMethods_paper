...
\newcommand{\jea}[1]{{\color{blue} \textbf{ #1}} }
...
% First, explain the estimator and what it is supposed to do.
% Then show that using empirical covariance naively will lead to problems.
% Then discuss what happens with not enough modes - here the convergence to the correct eigenvectors is discussed 
...
focus solely on Stokes I estimated data (rather than the full Stokes considered in 
\citep{moore_et_al2017})
%JEA - I didn't understand the original citation to Moore 2013
from PAPER's $30$ m East/West baselines (Figure 
...
as solid green. In the revised PAPER-64 analysis in this paper, we choose to use a regularization scheme of $\widehat{\textbf{C}}_{\rm eff} \equiv 0.035 \, $Tr($\widehat{\textbf{C}})\textbf{I} + \widehat{\textbf{C}}$ ($\gamma = 0.035$) as a simple example of regularization that minimizes loss, and note that the power spectrum limits are roughly constant across a large range of values of $\gamma$.}
