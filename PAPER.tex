...
web of galaxies we observe today \citep[for a review, see][]{furlanetto_et_al2006}.
...
the accuracy and reliability of a potential EoR detection. In this paper, we discuss three topics that are essential to investigate 
...
weighting that also takes into account frequency correlations (\citealt{liu_tegmark2011}; \citealt{dillon_et_al2013a}; \citealt{liu_et_al2014a}; \citealt{liu_et_al2014b}; \citealt{dillon_et_al2014}; \citealt{dillon_et_al2015}). This weighting has the effect of down-weighting correlated 
...
We begin with our data vector, $\textbf{x}$, which contains our measured visibilities for a single baseline in Jy. It has length $N_{t} \times N_{f}$, but in practice we manipulate it as an array with dimensions ($N_{t}, N_{f}$), where 
...
Kelvin and taking into account cosmological scalings. It is evaluated as $\textbf{Q}^{\alpha} \equiv \frac{\delta\textbf{C}}{\delta p_{\alpha}}$, or the derivative of the covariance with respect to the true bandpower in $\alpha$, where $\alpha$ denotes a waveband in $k_{\parallel}$ ($k_{\parallel}$, a cosmological wavenumber, is the 
...
\textbf{M}$ (subject to the constraint that window functions are sensible) --- for simplicity in this section we choose $\textbf{M}$ to be diagonal, although we explore other cases for the analysis 
...
matrices $\textbf{R}$ and examine their impact on the resulting power spectra estimate $\widehat{\textbf{p}}$. Our goal in 
...
\textbf{x}_{FG} + \textbf{x}_{EoR}$, to which we apply different weighting schemes. The three data components are shown in Figure 
...
One choice for the weighting matrix $\textbf{R}$ is an inverse covariance matrix. This type of weighting is attractive for power spectrum analyses because it yields the smallest possible error bars on a measurement. Said differently, it gives the minimum variance estimate of the power spectrum of a signal (\citealt{tegmark_et_al1997a}; \citealt{bond_et_al1998}). In \citet{liu_tegmark2011}, it was shown that inverse covariance weighting also serves as a way to down-weight foregrounds since foregrounds have large covariances in frequency, motivating our use of the weighting for the PAPER analysis.

The covariance matrix $\textbf{C}$, which describes covariances between frequency channels, can be estimated in a variety of ways. Given perfect foreground, 
...
Therefore, one attractive way to estimate $\textbf{C}$ is to empirically derive it from the data $\textbf{x}$ itself. For instance, one could time-average such that:
...
amount of time), where $\langle \rangle_{t}$ denotes a finite average over time. The weighting matrix is therefore $
\textbf{R} = \widehat{\textbf{C}}^{-1}$. 

First, we compute the power spectrum of $\textbf{x}$ using OQE formalism and a weighting matrix of $\widehat{\textbf{C}}^{-1}$. 
...
case, signal loss arises from using information from all eigenmodes of $\widehat{\textbf{C}}$. Because $\widehat{\textbf{C}}$ is 
empirically estimated, its eigenvalues differ from those of the true covariance and this error primarily affects the weaker (EoR-dominated) eigenmodes.
There is negligible signal loss when assigning identical weights to all eigenmodes except the first, since we are not using the relative weights of the weaker eigenmodes 
which $\widehat{\textbf{C}}$ does not describe accurately.}
...
%, which is shown in Figure \ref{fig:toy_sigloss12}.
...
If the covariance is estimated from our data, its eigenspectrum differs from the eigenspectrum of the true covariance $\textbf{C}$, and this difference has consequences on our result. An eigenspectrum ranks the eigenvalues of a matrix from 
...
for which a sloped eigenspectrum means that modes are given different weights in OQE power spectrum estimation. The modes with the highest eigenvalues are 
...
In general, the strongest modes of $\widehat{\textbf{C}}$ (highest eigenvalues) are expected to be dominated by bright foregrounds 
--- the most prominent shapes in the data. For these `strong' modes, where foregrounds outshine the EoR signal, $
\widehat{\textbf{C}} \sim \textbf{C}$ and down-weighting these modes is beneficial. This is demonstrated in the toy model by the 
...
$\widehat{\textbf{C}}_{FG}$ but effectively absent from $\widehat{\textbf{C}}^{-1}_{FG}\textbf{x}_{FG}$.
...
\textbf{C}$ accurately, for which the EoR signal is brighter than foregrounds. In such a case, the empirical covariance believes these modes should be down-weighted more than the true covariance would. For these modes, the error that results from not computing $\textbf{C}$ accurately (due to a finite data sample), has a larger impact on the `weak' modes than the `strong' modes. In other words, mis-estimating $\textbf{C}$ affects EoR-dominated eigenmodes more than FG-dominated modes, and it is primarily the error associated in down-weighting these `weak' modes that leads to signal loss. 
...
project out the first eigenmode (i.e. use the true eigenvalue of the first eigenmode, but flatten the rest of the spectrum to have eigenvalues of 1), thereby down-weighting the foreground-dominated mode more than the rest of the modes. Hence, we are not using the relative weighting information from all the weaker eigenmodes which have comparatively larger errors. Altering $\widehat{\textbf{C}}$ as such is one example of a regularization method, in which we are 
changing $\widehat{\textbf{C}}$ in a way that flattens its eigenspectrum, making it more similar to that of $\textbf{I}$. The resulting 
...
them without signal loss. There are several other ways to regularize $\widehat{\textbf{C}}$, and we will discuss some in Section 
\ref{sec:otherweight}.
...
Rather than a traditional box-car average, a time domain filter can be designed to up-weight temporal modes consistent with 
...
\cc{Note to Adrian - somewhere in the next paragraph would be good to ref Fig17}
...
To quantify the convergence of `weak' and `strong' eigenmodes present in a sloped eigenspectrum, we form a mock covariance matrix with diagonal values that span $4$ orders of magnitude. We then draw random data realizations from this covariance and compute a convergence metric $\varepsilon(\widehat{\textbf{v}})$ for the empirical eigenvectors $\widehat{\textbf{v}}$:
\begin{equation}
\label{eq:converge_eig}
\varepsilon (\widehat{\textbf{v}}) \equiv \frac{1}{N_{f}}\sum_{i}^{N_{f}}|\textbf{v}-\widehat{\textbf{v}}|_{i},
\end{equation}
where $N_{f}$ is the number of frequencies ($20$) in the mock data. We plot this statistic against the number of realizations in Figure \ref{fig:toy_sigloss17}, which shows that...
...
true covariance matrix. This can be quantified by evaluating a convergence metric $\varepsilon(\Chat)$ for the empirical covariance, which we define as
\begin{equation}
\label{eq:converge}
...
true and modeled covariance matrices. In practice such a weighting option is not feasible, as it is difficult to model $\textbf{C}_{EoR}$, and $\textbf{C}_{FG}$ is unknown.
...
similar weights, we avoid down-weighting EoR modes more than others. Although there is negligible signal loss, the small green peak at $k\sim3$ represents residual foregrounds that still exist since the shapes encoded in the off-diagonal frequency correlations of the covariance matrix were deemed not as prominent as the diagonal elements using this weighting scheme. 

The third panel (bottom left) in Figure \ref{fig:toy_sigloss8} flattens out the eigenspectrum of $\widehat{\textbf{C}}$ a different way - 
by only using the first three eigenmodes. Recalling that our toy model foregrounds can be described entirely by the first eigenmode, this 
...
panel of Figure \ref{fig:toy_sigloss8}, we see that this method does not down-weight the foregrounds. For this toy model, 
...
suppressing the foreground. But because we flattened the eigenspectrum, we also avoid signal loss. Although this method did not successfully recover EoR for this particular simulation, it is important that we show that there 
...
spreading to high $k$ values. For example, narrow window functions in delay-space can be used to minimize the covariance of a particular 
...
Determining the source of detections at higher $k$'s is more difficult. In previous power spectrum results, these detections have been explained as instrumental systematics, particularly time-variable cross talk, RFI, cable reflections, and calibration errors (A15; \citealt{parsons_et_al2014}; \citealt{dillon_et_al2014}; \citealt{beardsley_et_al2016}; \citealt{patil_et_al2017}). In the next section (Section \ref{sec:JackknifeOverview}), we will present some tests that can help distinguish these excesses from that of EoR. 
...
itself. Here we describe a methodology for estimating the amount of signal loss caused by a particular power spectrum estimator when applied to a particular data set. There is, of course, no way of determining an exact signal loss ``correction factor," i.e., we cannot perfectly undo the effects of a lossy estimator.
...
likely due to the use of the fringe-rate filter as a simulator. However, this pipeline difference, while significant, is not the dominant reason why signal loss was under-estimated in A15 (the dominant reason is explained in the main text).}
...
\end{eqnarray}
where, for illustrative purposes and notational simplicity, we have written these equations with diagonal normalization matrices $M$, even though for our numerical results we use a full non-diagonal matrix $\mathbf{M}$ as in Equation \eqref{eq:phat}. $P_{\rm in}$ represents the unweighted power spectrum of $\textbf{e}$, our simulated EoR signal, 
...
In Section \ref{sec:Illustration}, we provide some intuition for how the transfer function is able to capture signal loss, and why this signal loss occurs in the first place. Section \ref{sec:Practice} then details the numerical computations used to translate our power spectrum result into one viewed through a 
...
To explore how our expression for $P_{out}$ encapsulates signal loss, we expand out Equation \eqref{eq:sigloss}:
...
^{\alpha}\textbf{R}_{x}\textbf{x}.
\end{eqnarray}
We see that $P_{\rm out}$ is comprised of multiple terms. For illustrative purposes in this subsection only, we build intuition for signal loss in $P_{out}$ by considering a scenario where $\textbf{e}$ dominates compared to $\textbf{x}$. It is in this limit where we expect signal loss to be greatest, and $P_{out}$ becomes:
...
\end{equation}
(Note that even this is an approximation. In principle there is an infinite series containing all possible powers of $e$ and $f$). Now, we split $f$ into $ f_\parallel + f_\perp$, which are the components of the foregrounds that couple to the EoR versus those that do not. Further parameterize $f_\parallel \equiv \rho e$, where $\rho$ is some correlation coefficient that quantifies the degree of correlation between the foregrounds and the EoR. Under this parameterization, one can write the $fe$ term in terms of $e^2$ (which gets absorbed into the other EoR term, plus residual foregrounds that we don't care about).}
...
In our code, we simulate $20$ realizations of $P_{in}$ by bootstrapping over baselines, as explained in Section \ref{sec:Boot}, yielding $P_{\rm in}$ values that range from $\sim$$10^{2}$ mK$^{2}$ (h$^{-1}$ Mpc)$^{3}$ to $\sim$10$^{16}$ mK$^{2}$ (h$^{-1}$ Mpc)$^{3}$. We also run $25$ total EoR injection levels, resulting in a total of $500$ data points on our $P_{\rm in}$ vs. $P_{\rm out}$ grid. 
...
weighted case (red), and added-identity case (cyan); all are after signal loss estimation. All three regularizations shown (black, red, cyan) perform similarly (when 
...
which denote power spectrum values, post-signal loss estimation, for two other regularization schemes (these are single-valued and not a function of the number of eigenmodes down-weighted). We multiply an identity 
matrix element-wise to $\widehat{\textbf{C}}$ (i.e. inverse variance weighting, shown in red), and also add an arbitrarily chosen level 
of $\textbf{I}$ to $\widehat{\textbf{C}}$ (cyan). We see that all three regularization schemes (black, red, cyan) perform similarly at 
their best (i.e. when $\sim3$$-4$ eigenmodes are down-weighted in the case of the black curve).

For the remainder of this paper, we choose to use the weighting option of $\widehat{\textbf{C}} = \widehat{\textbf{C}} \circ \textbf{I}$, or 
inverse variance weighting, which we will denote as $\widehat{\textbf{C}}_{eff}$. We choose this weighting scheme due to its 
...
\begin{figure*}
	...
	\caption{Power spectrum of PAPER-64 using $\widehat{\textbf{C}}_{eff}$ (inverse variance weighting). Black and gray points correspond to positive and 
...
estimates after squaring visibility measurements. In A15, a factor of $2$ was mistakenly used.
...
one major changes in the way we estimate errors since A15, using the lesson we've learned about bootstrapping independent samples.
...
PAPER-64 power spectrum (one baseline type only) with weighting matrix $\widehat{\textbf{C}}_{eff}$. There are clear detections at 
low $k$'s.
...
estimated error region is LST-dependent, and therefore likely to be residual foregrounds. In other words, $t_{1}$ and 
...
In the main text we made an estimate of the signal loss effecting only the injected EoR by supposing that the lossy injected signal was the difference between the injected and not injected cases: $P_{out} = \widehat{\textbf{p}}_{r} - \widehat{\textbf{p}}_{x}$. Suppose that we rephrase the question we are asking to: ``At what point would an injected signal be detectable at significant levels?". Instead of disentangling the part of the output power spectrum dominated by the injected signal we can simply use $\widehat{\textbf{p}}_{r}$, the weighted power spectrum of the data plus injected signal.
...
\begin{equation}
\widehat{\textbf{p}}_{r} > \widehat{\textbf{p}}_{x} \Lambda,
\end{equation}  
where $\Lambda$ represents a choice of threshold?'' Of course, both sides of this equation are actually a distribution of values associated with data variance and injected model variance. One way to interpret this test for the two distributions is to compute the probability that the two distributions are not (i.e. the null test) the same.  
 
 \begin{equation}
\Prob(P_{in})  =  1 -  \int_{r=x} \Prob(\widehat{\textbf{p}}_x) \Prob(\widehat{\textbf{p}}_r,P_{in}) dr
 \end{equation}
 where $\Prob(\widehat{\textbf{p}}_x)$ is the probability of obtaining the power spectrum $\widehat{\textbf{p}}_x$ found via bootstrapping (power spectrum of data only) and $\Prob(\widehat{\textbf{p}}_r,P_{in})$ is the probability of obtaining the summed power spectrum $\widehat{\textbf{p}}_{r}$ given a range of input power levels, as sampled in Figure \ref{fig:Pr_vs_Pin}.  
...
%\dcj{checked with a little python test but needs a cite} 
...
The resulting limits for inverse covariance weighted PAPER-64 data (the same data used in Section \ref{sec:Sigloss}, no regularization), using this second signal loss method, are shown in Figure \ref{fig:sigloss_compare} compared with the method used in the main text. Overall the limits show good agreement, with k modes differing by factors of 2 to 4.  
