...
\cc{THIS SECTION IS LARGELY INCOMPLETE.}

\cc{Goal of this section is to showcase our signal loss transfer function for PAPER-64 and describe how we compute our signal loss factors.}

\cc{The signal loss methodology is still being worked on but is almost done - it should be straightforward to type it up after.}

\cc{Also include text describing Figure \ref{fig:sigloss_modeloop} which is an interesting one}
...
\begin{figure}
	\centering
	\includegraphics[width=\columnwidth]{plots/sigloss_modeloop.png}
	\caption{\cc{fill out caption}}
...
\cc{THIS SECTION IS ALSO LARGELY INCOMPLETE.}
...
The full inverse covariance treatment of our data is suboptimal to the unweighted case, meaning that choosing such a weighting does not help suppress systematics as we intended. Our ultimate goal is to find a weighting method that does successfully down-weight contaminants in our data and make some improvement over the unweighted power spectrum. There are many choices for determining the covariance matrix $\textbf{C}$, as described in Section \ref{sec:SiglossOverview}, but here we will illustrate \cc{some number?} promising ones as applied to PAPER-64.

\cc{To Do: Insert PS results here that show different regularization schemes}

\cc{ALSO: this is where our final revised limit will likely be! (Fig \ref{fig:ps1_data})}
...
\item $N_{seps}$: The number of baseline separation types averaged incoherently in a final power spectrum estimate. For the analysis in this paper, we only use one type of baseline, hence $N_{seps}=1$. The revised limit in \cc{cite retraction paper} uses three separation types.
