...
\begin{equation}
\label{eq:OQELinear}
\widehat{b}_{\alpha} = \half \Tr\left[ (\C - \mathbf{S}) \invC \PDeriv{\C}{p^\alpha} \invC \right].
\end{equation}
...
Motivated by the desire to retain the desirable behavior of suppressing contributions of $\mathbf{U}$ to estimates of the EoR power spectrum, we note that is possible to define a more generalized version of the quadratic estimator 
