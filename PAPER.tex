...
We smooth the 2D distribution of $P_{in}$ vs. $P_{out}$ using kernel density estimators\citep{scott2008kernel}. 
...
the shape of the transfer curve is nearly identical for all $k$'s (though we treat each $k$ separately).Bayes' rule 
then simply instructs us compute $p(\phat_{x})$, multiply it horizontally across Figure 
\ref{fig:sigloss_transfercurve} horizontally as defined in Equation \eqref{eq:Bayes} to get $p(P_{in})$. The final 
result can then be normalized to give $p(P_{\rm eor} | \phat_{x})$. 
\dcj{Need to write down the normalization for \eqref{eq:Bayes}}
The width of the distribution of  $\phat_{x}$ itself is defined as a  gaussian with the mean $\phat_{x}$ and 
variance determined from the bootstrap process. 


Inspection of the transfer function plot in Figure \ref{fig:sigloss_transfercurve} gives a sense of what the result should be.  The peak of our data distribution $
