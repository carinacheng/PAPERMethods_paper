...
In our code, we simulate $20$ realizations of $\p_{\rm eor}$ by bootstrapping over baselines, as explained in Section \ref{sec:Boot}, yielding $P_{\rm in}$ values that range from $\sim$$-10^{13}$ mK$^{2}$ (h$^{-1}$ Mpc)$^{3}$ (negative $P_{\rm in}$ is injected when one copy of \textbf{e} in Equation \eqref{eq:Pin} is negative and the other is positive) to $\sim$10$^{13}$ mK$^{2}$ (h$^{-1}$ Mpc)$^{3}$. We also run $40$ total EoR injection levels, resulting in a total of $800$ data points on our $P_{\rm in}$ vs. $P_{\rm out}$ grid. 
