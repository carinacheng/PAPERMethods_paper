...
It is clear that in order to obtain an unbiased estimate in the presence of contaminants, the estimator Equation \ref{eq:OQE} must be capable of ``suppressing'' or ``removing'' their effects.   The way in which this comes about is illustrated with a toy model in Appendix \ref{sec:icw_appendix}.  There we show that the effect of the weighting in Equation \ref{eq:OQEQuadratic} is to project out the modes of $\textbf{U}$ with a different covariance structure than $\textbf{Q}^\alpha$ in the power spectrum estimate, and the effect of Equation \ref{eq:OQELinear} is to subtract out the remaining bias.  


if the covariance of the contaminant is known perfectly, and its covariance structure is sufficiently different from the EoR signal, then 

we can expect a quadratic estimator of the form Equation \ref{} % Need to put the full equation in the main body, I think
...
In \citet{liu_tegmark2011}, it was shown that inverse covariance weighting also serves as a way to down-weight some portion of foregrounds (namely, those which do not share the same covariance structure as the cosmological signal), motivating our use of the weighting for previous PAPER analyses.

%We do this when forming the un-normalized power spectrum estimate $\widehat{q}_{\alpha}$:
It is also possible to define a more generalized version of this minimum-variance, unbiased estimator by
\begin{equation}
\label{eq:qhat}
