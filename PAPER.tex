...
\begin{figure*}
	...
	\caption{Power spectrum results (with $2\sigma$ errors) for two null tests, where a jackknife is taken along the LST axis (left) and baseline axis (right). The results shown are unweighted. The gray shaded region is the estimated $2\sigma$ theoretical noise limit given the parameters of each test. We find that there are no significant systematics that differ between the two tests, though the $k$-dependence of the errors suggest that we are dominated by residual foregrounds near the horizon.}
	\label{fig:null}
\end{figure*}
