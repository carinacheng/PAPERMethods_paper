...
The relationship between the input and output power spectra, $P_{in}$ and $P_{out}$, can be thought of as a transfer function which, for a sampling of $P_{in}$ and $P_{out}$ provides a mapping from an output power spectrum distribution into an input distribution. By viewing data through this signal loss lens, we are asking the question ``what input power spectrum distribution could this (signal-loss affected) data come from?" In the next section, we describe our signal loss transfer function for PAPER-64 data and detail the computation used to translate our power spectrum result into one viewed through a signal loss lens.
