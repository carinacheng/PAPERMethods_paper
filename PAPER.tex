...
The even/odd test shows $81\%$ of all error bars being consistent with zero, with several power spectrum points falling outside the estimated error region. Rather than attribute these biases to systematics that differ \textit{between} the `even' and `odd' datasets, we believe that they are caused by the squaring of thermal noise (which was allowed in the construction of this test). This supports our choice to not allow the cross-multiplication between the same LST-binned datasets in our full pipeline.
