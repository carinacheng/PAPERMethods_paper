...
The first term of $P_{\rm out}$ is simply the weighted power spectrum of $\textbf{e}$ alone. It is this quantity that was used in P64 to calculate signal loss. However, without taking into account the cross-terms (the second and third terms), we can substantially under-estimate loss. In short, this is because correlations between the data and injected EoR signal emerge as statistical noise that is added to the power spectrum estimate, decreasing the value of $P_{\rm out}$. Including these cross-terms in our new analysis represents the bulk change of our revised analysis compared to P64.

A deep investigation of the cross-terms reveals that they contain non-negligible power (when $\textbf{R}_{r}$
is taken to be $\textbf{C}_{r}^{-1}$ and $\textbf{C}_{r}$ is estimated from the data itself). We now describe in words the anti-correlation between data and the injected EoR. For a full...
