...
MHz ($20$ channels), identical to the analysis in A15. We note that, besides using only one baseline type instead of the three in 
A15, the PAPER-64 dataset that we use in this case study differs from that in A15 mainly by the applied fringe-rate filter. In A15, the 
...
estimate of signal loss (by a factor of $\sim$$10$), likely due to the use of the fringe-rate filter as a simulator. However, this pipeline difference, while significant, is not the dominant reason why signal loss was under-estimated in A15 (the dominant reason is explained in the main text).}
...
In short, we can summarize a signal loss estimate as the ratio of $P_{\rm out}/P_{\rm in}$, evaluated at the data level $\widehat{\textbf{p}}_{x}$. We motivate the fact that we can evaluate the output-to-input power spectrum ratio at $\widehat{\textbf{p}}_{x}$ by the following reasoning.
...
P_{\rm out, \alpha} &\approx&  M_{\alpha}\textbf{e}^{\dagger}\textbf{R}_{r}\textbf{Q}^{\alpha}\textbf{R}_{r}\textbf{e} +
 2M_{\alpha}\textbf{x}^{\dagger}\textbf{R}_{r}\textbf{Q}^{\alpha}\textbf{R}_{r}\textbf{e}.
\end{eqnarray}
...
P_{\rm out,\alpha} &=&  M_{\alpha}\textbf{r}^{\dagger}\widehat{\textbf{C}}_{r}^{-1}\textbf{Q}^{\alpha}\widehat{\textbf{C}}_{r}^{-1}\textbf{r} \nonumber \\
&&\phantom{=====} 
-  M_{\alpha}\textbf{x}^{\dagger}\widehat{\textbf{C}}_{x}^{-1}\textbf{Q}^{\alpha}\widehat{\textbf{C}}_{x}^{-1}\textbf{x} 
\end{eqnarray}
...
down-weight. Fundamentally, each of these examples are similar in that they rely on a computation of $\widehat{\textbf{C}}$ from 
