...
\phat_{x})$ for $\p_{\rm eor}$ given the uncorrected power spectrum estimate $\phat_{x}$. Given our ansatz from Section \ref{sec:siglossmethod} that $P_{\rm out}$ is a good estimate of $\p_{x}$, we can write
...
\noindent where $p(P_{\rm out})$ is the probability of the measured value (given our error) and $\mathcal{L} $ is the likelihood function defined 
