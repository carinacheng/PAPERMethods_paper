...
\altaffiltext{6}{Square Kilometer Array, S. Africa, Cape Town South Africa}
\altaffiltext{7}{Dept. of Physics and Electronics, Rhodes University, South Africa}
\altaffiltext{8}{INAF-Instituto di Radioastronomia, Bologna Italy}
\altaffiltext{9}{Dept. of Electrical and Computer Engineering, U. Virginia, Charlottesville VA}
\altaffiltext{10}{National Radio Astronomy Obs., Charlottesville VA}
\altaffiltext{11}{Dept. of Astronomy, U. Virginia, Charlottesville VA}
...
learned from previous experiments and is forecasted to be able to make a high-significance power spectrum 
...
estimation technique of optimal quadratic estimators (OQE) as done in \citet{liu_tegmark2011}, \citet{dillon_et_al2013a}, \citet{liu_et_al2014a}, \citet{liu_et_al2014b}, \citet{trott_et_al2012}, and \citet{trott_et_al2016}. Before showing 
how signal loss can arise when using different weighting matrices, we first summarize OQE as performed in the PAPER 
analysis.

We begin with our data matrix, $\textbf{x}$, which contains our measured visibilities for a single baseline in Jy. It has dimensions ($N_{t}, N_{f}$), where 
...
Kelvin, as well as cosmological scalings. It is evaluated as $\textbf{Q}^{\alpha} \equiv \frac{\delta\textbf{C}}{\delta p_{\alpha}}$, or the derivative of the covariance with respect to the true bandpower in $\alpha$, where $\alpha$ denotes a waveband in $k_{\parallel}$ ($k_{\parallel}$ is the 
...
amount of time), where $\langle \rangle$ denotes a finite average over time $t$. The weighting matrix is therefore $
...
There is no signal loss when using only the first eigenmode, since we are not using information from the weaker eigenmodes 
...
\textbf{C}$ accurately, for which the EoR signal is brighter than foregrounds. For these modes, the error that results from not computing $\textbf{C}$ accurately (due to a finite data sample), has a larger impact on the `weak' modes than the `strong' modes. In other words, mis-estimating $\textbf{C}$ affects EoR-dominated eigenmodes more than FG-dominated modes, and it is primarily the error associated in down-weighting these `weak' modes that leads to signal loss. 
...
project out the first eigenmode (i.e. zero out all eigenmodes of the covariance matrix except the first one), thereby down-weighting the foreground-dominating mode only. Hence, we are not using information from all the weaker eigenmodes which have comparatively larger errors. Altering $\hat{\textbf{C}}$ as such is one example of a regularization method, in which we are 
...
\begin{equation}
\Delta^{\textbf{2}}(k) = \frac{k^{3}}{2\pi^{2}}\hat{\textbf{p}}\textbf{(k)},
\end{equation}
...
In short, we can summarize a signal loss estimate as the ratio of $P_{\rm out}/P_{\rm in}$, evaluated at the data level $\hat{\textbf{p}}_{x}$. We motivate the fact that we can evaluate the output-to-input power spectrum ratio at $\hat{\textbf{p}}_{x}$ by the following reasoning.
...
When $\textbf{R}_{r}$
is taken to be $\hat{\textbf{C}}_{r}^{-1}$, the second term of Equation \eqref{eq:pout_expand} is a cross-correlation between $\hat{\textbf{C}}_{r}^{-1}\textbf{x}$ and
$\hat{\textbf{C}_{r}}^{-1}\textbf{e}$. As shown in \citet{switzer_et_al2015}, this cross-correlation term is non-zero, and in fact negative in expectation. 
This negative cross-term power arises from a coupling between the inverse of 
$\hat{\textbf{C}}_{r}$ and $\mathbf{x}$. 
...
\begin{eqnarray}
\hat{\textbf{C}}_{r} &=& \langle \textbf{rr}^{\dagger} \rangle_{t} \nonumber \\ 
...
inversion of a matrix like $\hat{\textbf{C}}_{r}$ that is diagonal-dominant (from $\langle 
...
terms depend on both $\textbf{x}$ and $\textbf{e}$. When $\hat{\textbf{C}}^{-1}_{r}$ is multiplied into $\textbf{x}$,
the result is a vector that is similar to $\textbf{x}$ but
contains a residual correlation to $\textbf{e}$ from the off-diagonal components of $\hat{\textbf{C}}^{-1}_{r}$. The
correlation is negative because the product $\hat{\textbf{C}}_r^{-1}\textbf{x}$ effectively squares the $\textbf{x}$-dependence
of the off-diagonal terms in $\hat{\textbf{C}}^{-1}_{r}$ while retaining the negative sign that arose from the inversion
of a diagonal-dominant matrix.
% XXX [ARP: we may want to write this all out explicitly in an appendix]

The correlation between $\hat{\textbf{C}}_r^{-1}\textbf{e}$ and $\hat{\textbf{C}}_r^{-1}\textbf{x}$ means that we cannot compute signal loss using a signal-only 
...
We thus see from this subsection that signal loss generically arises because of couplings between $\hat{\textbf{C}}^{-1}_{r}$ and $\mathbf{x}$. In principle, using $\hat{\textbf{C}}^{-1}_{r}$ in place of the true inverse covariance $\textbf{C}^{-1}$ provides yet another source of multiplicative bias, which results in a modification to the treatment shown here. Such a modification is accounted for in the demonstration of Appendix \ref{sec:sigloss_appendix}, where we additionally discard the assumption that $|\textbf{x}|\ll|\textbf{e}|$. However, despite the loosening of these approximations, one sees that the qualitative message (of anti-correlations resulting in signal loss) remains intact.
...
($\textbf{R} = \hat{\textbf{C}}^{-1}$), the weighting scheme used in A15 which, with an empirically estimated covariance, leads to substantial loss. With this weighting, our 
...
P_{\rm out,\alpha} &=&  M_{\alpha}\textbf{r}^{\dagger}\hat{\textbf{C}}_{r}^{-1}\textbf{Q}^{\alpha}\hat{\textbf{C}}_{r}^{-1}\textbf{r} \nonumber \\
&&\phantom{=====} 
-  M_{\alpha}\textbf{x}^{\dagger}\hat{\textbf{C}}_{x}^{-1}\textbf{Q}^{\alpha}\hat{\textbf{C}}_{x}^{-1}\textbf{x} 
\end{eqnarray}
...
bootstrapping step along time entirely and now simply bootstrap over baselines. Power spectrum estimates with this bootstrapping change for PAPER-64 fringe-rate filtered data are shown in Figure 
...
independent samples (by over-sampling elements along the time axis), repeated values begin to crop up and the apparent variation between bootstraps drops, resulting in limits below the predicted noise level (gray).

\begin{figure}
	\centering
	\includegraphics[trim={0.3cm 0cm 0.3cm 0.3cm},width=\columnwidth]{plots/data_errors.png}
	\caption{$2\sigma$ power spectrum errors (not upper limits) using PAPER-64 data and two different bootstrapping 
...
In the main text we made an estimate of the signal loss effecting only the injected EoR by supposing that the lossy injected signal was the difference between the injected and not injected cases: $P_{out} = \hat{\textbf{p}}_{r} - \hat{\textbf{p}}_{x}$. Suppose that we rephrase the question we are asking to: ``At what point would an injected signal be detectable at significant levels?". Instead of disentangling the part of the output power spectrum dominated by the injected signal we can simply use $\hat{\textbf{p}}_{r}$, the weighted power spectrum of the data plus injected signal.
