...
led to a revised, higher $21$ cm power spectrum upper limit from PAPER-64. This revised result, presented in a companion paper by Kolopanis et al., \textit{in prep}, mostly stems from an 
...
%A third option which acts as a middle ground between the other two is 
...
the amount of signal loss accompanying a weighting scheme used in OQE power spectrum estimation (an approach also taken by \citet{masui_et_al2013}).  
The results of such an injection scheme can be interpreted in multiple ways.  In this paper we describe two. This section describes one and in Appendix \ref{sec:Pr_appendix} we present an alternate interpretation.
...
The gist of our method is to inject a eor-like signal into the data and then measure the amount recovered, with the difference being accounted as signal loss.  To capture the full statistical likelihood of signal loss, one requires a quick way to generate many realizations of simulated $21$ 
...
data plus injection, and $P_{\rm out}$ is the difference between $\widehat{\textbf{p}}_{r}$ and $\widehat{\textbf{p}}_{x}$.  In the limit where no significant cross correlations between the data and the injected signals are found, this will recover the amount of injected power spectrum remaining after the covariance weighting operation, ie the amount of background EoR power $P_{\rm eor}$ not removed by the foreground downweighting procedure. In other words, we make the anzatz that 

\begin{equation}
P_{\rm out} = P_{\rm eor} \label{eqn:anzatz},
\end{equation}
folding in our definition from Equation \ref{eq:sigloss}, this definition implies that we are seeking the injected 
signal amplitude which causes a power doubling ($\phat_r = 2 \phat_x$).
...
Assuming R$_{r}$ is symmetric the two cross terms can be summed together as 

\begin{eqnarray}
\label{eq:crossterm}
P_{\rm out,\alpha} &= &  M^{\alpha}_{a}\textbf{x}^{\dagger}\textbf{R}_{r}\textbf{Q}^{\alpha}\textbf{R}_{r}\textbf{x} \\
&+& M^{\alpha}_{b}\textbf{e}^{\dagger}\textbf{R}_{r}\textbf{Q}
^{\alpha}\textbf{R}_{r}\textbf{e} \nonumber \\
&+& 2 M^{\alpha}_{c}\textbf{x}^{\dagger}\textbf{R}_{r}\textbf{Q}^{\alpha}\textbf{R}_{r}\textbf{e} \\
&-& M^{\alpha}_{x}\textbf{x}^{\dagger}\textbf{R}_{x}\textbf{Q}
^{\alpha}\textbf{R}_{x}\textbf{x}.
\end{eqnarray}.


Each of these terms is plotted in Figure \ref{fig:sigloss_terms} for two cases: inverse covariance weighting ($\textbf{R}_{r} = \widehat{\textbf{C}}_{r}^{-1}$) and unweighted ($\textbf{R}_{r} = \textbf{I}$).   Let us examine the behavior of this equation in the regimes of very weak and very strong injected signal.


{\bf Small injection}
The first and the fourth term are very similar, differing only in their covariance matrices. When the injected signal 
is very small compared to the data, their difference can be neglected.  In this limit we might imagine that the 
cross correlation should also be small enabling us to recover just the EoR term, however examining the cross-
term plotted in Figure  \ref{fig:sigloss_terms} we see that cross terms are still large enough to have a significant 
impact. This is trivially demonstrated in the identity case where we even at small injects, large cross terms are 
evident.
...
{\bf In between}
When the injected signal is of a similar amplitude to the data by itself, the picture becomes murky.  We see that 
the weighted injected power spectrum component mirrors the input power indicating little loss, eventually 
departing from unity when the injected amplitude is well above the level of the data power spectrum. However 
the cross terms have nearly the same amplitude, but a negative sign. The sum of the two is substantially lower and substantially drives down the $P_{\rm out}$ estimator.
...
simulation, which would yield greater values for $P_{\rm out}$ and thereby underestimate signal loss. This argument is also made in \citet{switzer_et_al2015} in regards to estimating loss associated with foreground cleaning of Great Bank Telescope (GBT) data. Therefore, in our revised 
...
One issue to address is how one incorporates the randomness of $P_{\rm out}$ into our signal loss corrections. A different realization of the test signal is injected with each bootstrap run causing the output to vary according to noise as well as whether the injected signal looks more or less ``like'' the data.

Phrased in the context of Bayes' rule, we wish to find the posterior probability distribution of $p( \p_{\rm eor} | 
\phat_{x})$ for $\p_{\rm eor}$ given the uncorrected power spectrum estimate $\phat_{x}$. Given our guess from section \ref{sec:siglossmethod}, that the change in power caused by the injection injection$P_{\rm out}$ is a good estimate of the injected EoR power $P_{\rm eor}$ we can write

\begin{equation}
\label{eq:Bayes}
p(P_{\rm eor} | \phat_{x}) \propto \evalat[\Bigg]{\mathcal{L} (  P_{\rm out} | P_{\rm in})}{P_{\rm out}=P_{\rm x}}
  p(\phat_{x}) ,
\end{equation}

\noindent where $p(\phat_x)$ is the probability of the measured value (given our error) and $\mathcal{L} $ is the likelihood function defined 
as the distribution of data + injection ($P_{\rm out}$) given the injection $P_{\rm in}$ evaluated at P.  We construct this distribution  
...
whole process can then be repeated for a different value of $P_{\rm in}$.  \dcj{am I right about the normalization?}

\subsection{Implementation}
The implementation details of the injection process require some more detailed explanation.
In our code, we add a new realization to each independent bootstrap with the goal of simultaneously capturing variance due to noise and signal loss. To limit computing time we perform $20$ realizations of $P_{in}$, yielding $P_{\rm in}$ values that range from $\sim$$10^{2}$ mK$^{2}$ (h$^{-1}$ Mpc)$^{3}$ to $\sim$10$^{16}$ mK$^{2}$ (h$^{-1}$ Mpc)$^{3}$. We also run $25$ total EoR injection levels, resulting in a total of $500$ data points on our $P_{\rm in}$ vs. $P_{\rm out}$ grid. 
...
We smooth the 2D distribution of $P_{in}$ vs. $P_{out}$ using kernel density estimators\citep{scott2008kernel}. The result, for inverse covariance weighting, is shown in the left plot of Figure \ref{fig:sigloss_transfercurve}.  Although we only show figures for one $k$ value, we note that 
the shape of the transfer curve is nearly identical for all $k$'s (though we treat each $k$ separately).Bayes' rule then simply instructs us to fix $\phat_{x}$ at our measured value, essentially reading Figure \ref{fig:sigloss_transfercurve} horizontally and reinterpreting Equation \eqref{eq:Bayes} as a function of $p(P_{in})$. The final result can then be normalized to give $p(P_{\rm eor} | \phat_{x})$. If $\phat_{x}$ itself does not take on a single value but is itself a distribution (for our analysis, this comes from bootstrapping), then one simply repeats the process for every single point on the distribution of $\phat_{x}$, to find the distribution of $P_{\rm eor}$. In Figure \ref{fig:sigloss_transfercurve}, the peak of our data distribution $
...
heat-maps for the cases of inverse covariance weighted PAPER-64 data (left) and unweighted data (right). The dotted black 
...
The result is the loss-corrected power spectrum shown in Figure \ref{fig:ps2_data} which we can compare  to the original lossy result. There are a few important checks to examine.  We see that the unweighted 
power spectrum $2\sigma$ upper limit is identical in both panels. This is an important check, as we expect no signal loss for this case. Any difference here would indicate an error in the pipeline or discrepancy in the range of data included.  Prior to signal loss estimation, it 
is clear that the power spectrum is inconsistent with the (now revised) theoretical noise level prediction. 
Post signal loss estimation, the power spectrum errors are higher than both the theory and unweighted power spectrum, a consequence of weighting with a steep eigenspectrum where the `weak' modes are strongly coupled to the time-averaged fringe-rate filtered data. We elaborate on this point in the next section, as well as investigate alternate 
...
only the strongest $\sim3$-$4$ eigenmodes are down-weighted and the rest are given equal weights. For a similar discussion on projecting out modes (zero-ing out eigenmodes, rather than just ignoring their relative weightings as we do in this study), see \citet{switzer_et_al2014}. 
...
one major change in the way we estimate errors since A15, using the lesson we have learned about bootstrapping independent samples.
...
Investigating Figure \ref{fig:null}, we find that the baselines test is fully consistent with noise (all error bars pass through zero), and that the even/odd test is consistent with noise given the $2\sigma$ expectations ($95\%$ of the error bars are very close or passing through zero). We additionally note that the error bars for the baselines test (green) are especially small --- this is because the signal is effectively cancelled out prior to cross-multiplication (when forming baseline groups). 
...
given injection level P$_{in}$. The $\widehat{\textbf{p}}_{r}$ values that are plotted on the y-axis represent the weighted power spectrum of data plus the injected signal. It differs from that of Figure \ref{fig:sigloss_transfercurve} by not subtracting off the power spectrum of data alone (not subtracting off the second term in Equation \eqref{eq:sigloss}). We show kernel density estimations of the power spectrum transfer functions as colored heat-maps for the cases of inverse covariance weighted PAPER-64 data (left) and unweighted data (right). The dotted black 
