\documentclass[preprint2,numberedappendix,tighten]{aastex6}  
% USE THIS TO MAKE BIB, THEN FORMAT USING EMULATEAPJ
...
% Put all of the (O)QE / ICW theory here, rather than sprinkled throughout this section.

% Some things we never quite say as explicitly as I'd like:
% - "foreground suppression" happens because the foregrounds have a different covariance structure (Figure 4), and we 
% hope we pick this out by picking big eigenvalues
% - additive bias is a thing
...
The fundamental dependence of the covariance on the power spectrum band powers $p_\alpha$ is encoded in $\textbf{Q}^{\alpha} \equiv \frac{\partial\textbf{C}}{\partial p_{\alpha}}$.  In practice, it also implements a transform of the frequency-domain visibilities to 
$\mathbf{k}$ space, taking into account cosmological scalings, and converting the visibilities from Jy to Kelvin.

their Fourier conjugates, and converts from Jy to Kelvin and . It is formally evaluated as  (which has dimensions ($N_{f}$,$N_{f}$)), or the derivative of the covariance $\textbf{C} \equiv \langle\textbf{x}\textbf{x}^{\dagger}\rangle$ with respect to the true bandpower $p_\alpha$, where $\alpha$ indexes a waveband in $k_{\parallel}$ (a cosmological wavenumber $k_{\parallel}$ is the 
...
In the next three sections, we use toy models to investigate the effects of weighting matrices on signal loss by experimenting with different matrices $\textbf{R}$ and examining their impact on the resulting power spectrum estimates $\widehat{\textbf{P}}$. Our goal in experimenting with weighting is to suppress foregrounds and investigate EoR losses associated with it. We note that we 
...
\subsubsection{Toy Model: Empirical Inverse Covariance Weighting}
\label{sec:toymodel}

We will now build intuition into how empirically estimated inverse covariance weighting can give rise to signal loss through the use of a toy model.  We construct a simple dataset that contains visibility data with $100$ time 
...
% While a later section does show that the large eigenvalue modes converge to the eigenvectors quicker, it is _not_ true that modes with big eigenvalues are downweighted more.  It is the modes which have a structure different from dC/dp_alpha.  Our regularization actually forces both eigenvalues and eigenmodes to be like the "signal" part of the covariance.  Thus, I don't think the next two paragraphs are helping the explanation of what is going on.
...
Because $\widehat{\textbf{C}}$ is estimated from the data, its eigenvectors and eigenvalues are strongly coupled to the particular data realization that was used to compute it.  In this case, the eigenmode of $\widehat{\textbf{C}}$ with the largest eigenvalue describes the sinusoid foreground mode in the toy model (the peak in Figure \ref{fig:toy_sigloss3}).  In Figure \ref{fig:toy_sigloss12} we show the estimated covariances of our toy model datasets along with $\widehat{\textbf{C}}^{-1}$ weighted data. The foreground sinusoid is clearly visible in $\widehat{\textbf{C}}_{\rm FG}$.

%In general, 
We have made the assumption that the highest-valued eigenmodes of $\widehat{\textbf{C}}$ will typically (but not always) describe bright foregrounds, though we have not checked that indeed the eigenvectors do describe correlations between frequencies.
% --- the most prominent shapes in a dataset. 
%For these FG-dominating modes, where foregrounds outshine the EoR signal, 
If such modes do describe foregrounds, then down-weighting is beneficial. In some sense, we desire signal loss in this regime, if by `signal' we mean `foregrounds.'  In this case it is beneficial for these eigenmodes to be coupled to the data in a way that produces loss. For our toy model, the successful suppression of the foreground mode is demonstrated in Figure \ref{fig:toy_sigloss3} by the missing foreground peak in the weighted power spectrum estimate (green).
...
We illustrate the way in which the quadratic estimator, Equation \ref{eq:QuadraticEstimator}, is intended to estimate the power spectrum of EoR in the presence of contamination by other terms by considering the simpler, but similar, case of estimating the variance of data.  We have specifically chosen a toy model where the data covariance is diagonal, as we expect the EoR signal to be.  We assume we have $N$ data points $\Delta_i$ which are the sum of a desired signal $\sigma_i$ and some contaminant $\phi_i$
\begin{equation}
\Delta_i = \sigma_i + \phi_i
\end{equation}
with 
\begin{equation}
\langle \sigma_i \rangle = 0; \; \langle \sigma^2_i \rangle = s; \; {\rm and}~\langle \sigma \sigma^T \rangle = s I_{N \times N} \equiv S
\end{equation}
...
\langle \phi^2_i \rangle = f; \; 
\langle \phi \phi^T \rangle = f I_{N \times N} \equiv F; {\rm and}~ 
\langle \sigma_i \phi_j \rangle = 0
\end{equation}
...
The estimator for $s$ using the inverse covariance weighting prescription is
\begin{equation}
\label{eq:CaseOneICW}
...
\end{equation}
In this case, the quadratic estimator returns an entirely reasonable result, but, 
{\it since the covariance structure of the contaminant is identical to the signal}, it is only possible to achieve an unbiased estimate by using some additional information to estimate the additive bias and subtract it.

Now consider a case, chosen to be very similar to the toy model in \ref{sec:},  in which the data have an additive contaminant given by
\begin{equation}
\Delta_i = \sigma_i + \phi m_i
\end{equation}
where the properties of $\sigma_i$ are as before,
%\begin{eqnarray}
%\langle \sigma_i \rangle & = & 0 \\
%\langle \sigma^2_i \rangle & = & s \\
% \langle \sigma \sigma^T \rangle & = & s I_{N \times N} \equiv S
%\end{eqnarray}
but now $\phi$ is a random variable and $m_i$ is a fixed function of $i$ with
\begin{equation}
\langle \phi \rangle = 0; \;
\langle \phi^2 \rangle = f; \; 
 \langle \phi \phi^T \rangle = f m m^T \equiv F; \;
 m^T m = 1
\end{equation}
Here $m$ represents a mode which is correlated across many data points, with amplitude given by  $\phi$.

Here we can again calculate the estimator given by \ref{eq:} explicitly by using the Sherman-Morrison identity to invert the covariance matrix.  Defining
\begin{equation}
\xi \equiv \frac{f/s}{1+f/s}
\end{equation} 
we have
\begin{equation}
\invC %& = & \frac{1}{s} I - \frac{g}{1+g/s} \frac{1}{s^2} m m^T \\
=   \frac{1}{s} \left(I - \xi m m^T \right)
\end{equation}
and the virtue of weighting by $\invC$ becomes clearer: in the limit $f \gg s$, $\xi \to 1$, and this becomes
\begin{equation}
\invC  = \frac{1}{s} \left(I - m m^T \right)
\end{equation}
where $I - m m^T$ is the projection operator, projecting out $m$ from any any vector it acts on.   Proceeding to calculate the full estimator
\begin{equation}
\hat{s} = \half {\cal F}^{-1} \left( \Delta^T \invC \PDeriv{C}{s}  \invC \Delta \right) = \frac{ \Delta^T  (I -  (\eta^2 - 2 \eta)  m m^T)  \Delta}{N + \eta^2 - 2 \eta} 
\end{equation}
we note that the expectation is
\begin{equation}
\langle \hat{s} = 
\end{equation}
