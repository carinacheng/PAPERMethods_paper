...
the shape of the transfer curve is nearly identical for all $k$'s. This transfer function is the likelihood function in Bayes' rule, namely $\mathcal{L} (  P_{\rm out} | P_{\rm in})$.  We invoke the Bayes interpretation and re-interpret as it as the posterior $P(P_{\rm in}|P_{\rm out})$ where we recall that $P_{\rm out}$ is a model of our data and make a horizontal cut across at the data value $P_{\rm out} = P_{\rm x}$ to yield a posterior distribution for the signal. We compute the $95\%$ confidence interval (an upper limit on EoR) from these distributions.
