...
The power spectrum result for PAPER-64 using the updated analysis presented in this paper, shown in 
Figure \ref{fig:ps1_data}, is biased above the predicted noise level, particularly at low $k$ values. As discussed in Section \ref{sec:BiasTypes}, these biases
are most likely attributable to foreground leakage. The cause for biases at higher $k$ values is more difficult to pinpoint. Here we demonstrate three null tests performed on PAPER-64 data that aim to isolate systematics in the data and verify 
that they are not attributable to EoR.

The three null test results are shown in Figure \ref{fig:null}, with each test displaying the differenced power spectrum between two halves of a jackknife. We take jackknives along three different axes, described as the following:
...
\item Even/Odd (blue): We split our dataset into even and odd Julian days. We form power spectra for each separately, allowing for the cross-multiplication of `even' with `even', for example. Although we don't allow this in our standard pipeline, we include these same-dataset cross-multiplications to allow a power spectrum to be estimated from solely the `even' data (similarly for `odd'). If the same sky signal is in both the `even' and `odd' datasets, we expect it to cancel out.
\item Baselines (green): We split our dataset into two halves, where each contains half of the total baselines used in the 
analysis. There are no baselines repeated between the two datasets.
\item LST (magenta): We split our dataset into two halves based on LST, namely the first half (LSTs 0.5-4.6 hours) and second half (LSTs 
4.6-8.6 hours).
\end{itemize}
...
Investigating Figure \ref{fig:null}, we find that the baselines test is fully consistent with noise (all error bars pass through zero). Additionally, the differenced power spectrum values (black points) all lie within the gray-shaded estimated error region for high $k$'s. Therefore, we conclude that particular baselines are not to be blamed for systematics in our dataset.

For the LST test, $90\%$ of the error bars pass through zero, which is slightly low given the expected $95\%$. Additionally, not all power spectrum points lie within the gray-shaded region, even at high-$k$. This LST-dependent systematic is likely caused by residual foregrounds that vary in LST. In other words, the two jackknife halves differ by an amount greater than thermal noise and do not effectively cancel out the sky signal when differencing. 

The even/odd test shows $81\%$ of error bars being consistent with zero, with several power spectrum points falling outside the estimated error region. Rather than attribute these biases to systematics that differ \textit{between} the `even' and `odd' datasets, we believe that they are caused by the squaring of thermal noise (which was allowed in the construction of this test). This strengthens our choice to not allow the cross-multiplication between the same LST-binned datasets in our full pipeline.

Finally, we note that all three null tests exhibit error bars that are strongly $k$-dependent. The errors are largest near the horizon, which is expected since foregrounds are brightest in this region post-delay filtering. We also note that all three tests result in power spectrum errors that are larger than the estimated thermal noise, even at high-$k$. There are a couple possible explanations for this. Although we expect each cross-multiplication to have independent noise (except in the even/odd case), there is still the possibility of a noise-foreground coupling term that can introduce power. This cross-term would vary with foreground strength, therefore being consistent with the shape of the results. Alternatively, our estimate of the thermal noise may be too low, as all three tests seem to imply a larger, but consistent, estimate.
