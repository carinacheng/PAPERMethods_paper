...
Kelvin and taking into account cosmological scalings. It is formally evaluated as $\textbf{Q}^{\alpha} \equiv \frac{\partial\textbf{C}}{\partial p_{\alpha}}$ (which has dimensions ($N_{f}$,$N_{f}$)), or the derivative of the covariance $\textbf{C} \equiv \langle\vec{\textbf{x}}\vec{\textbf{x}}^{\dagger}\rangle$ with respect to the true bandpower $p_\alpha$, where $\alpha$ indexes a waveband in $k_{\parallel}$ (a cosmological wavenumber $k_{\parallel}$ is the 
Fourier dual to frequency under the delay approximation (\citealt{parsons_et_al2012b}), which is a good approximations for the short baselines that PAPER analyzes). $\textbf{R}$ is a weighting matrix with dimensions ($N_{f}$,$N_{f}$) --- as 
...
panel of Figure \ref{fig:toy_sigloss8}, we see that this method does not down-weight the foregrounds --- this regularization altered $\widehat{\textbf{C}}$ in a way where it is no longer coupled to either the ``strong" or ``weak" eigenmodes. For this toy model, 
...
two quantities that go into a measurement have separate noises that don't correlate with each other. We also note that if the noise level is known, this type of bias can be subtracted off, though this procedure is argued to be dangerous (\citealt{dillon_et_al2014}).
...
\begin{eqnarray}
\textbf{x}_{1} &=& \textbf{n}_{1} + \textbf{e}_{1} + \textbf{f} \\
\textbf{x}_{2} &=& \textbf{n}_{2} + \textbf{e}_{2}
\end{eqnarray}

\noindent For example, $\textbf{x}_{1a}$ represents a jackknife sample (first half of the data) for the first measurement. 
Similarly, $\textbf{x}_{2b}$ represents a jackknife sample (second half of the data) for the second measurement. We note that $\textbf{e}$ itself will be different in two jackknives taken along LST, but could be identical in other cases (such as a jackknife along even/odd Julian days).
...
For our revised analysis, we use $8.1$ hours of LST (RA $0.5$-$8.6$ hours) and $51$ total baselines (\citetalias{ali_et_al2015} uses a slightly longer RA 
...
empirically estimated inverse covariance weighted case used in \citetalias{ali_et_al2015} is on the left and the uniform-weighted 
...
P_{\rm out})$, which is the probability of $P_{\rm in}$ given the uncorrected/measured power spectrum estimate $P_{\rm out}$.  Bayes' rule relates the posterior, which we don't know, to the likelihood, which we can forward model. In other words,
...
The loss-corrected power spectrum result for empirically estimated inverse covariance weighted PAPER-64 data is shown in Figure \ref{fig:ps2_data} (black dashed), which we can compare to the original lossy result (black and gray points). %There are a few important checks to examine. First, we see that the uniform-weighted power spectrum $2\sigma$ upper limit (dashed blue) is identical in both panels. This is an important check, as we expect no signal loss for this case. Any difference here would indicate an error in the pipeline or discrepancy in the range of data included. Additionally, it is clear that the power spectrum, prior to signal loss estimation, is inconsistent with the (now revised) theoretical noise level prediction. 
Post-signal loss estimation, the power spectrum limits are higher than both the theory (green) and uniform-weighted power spectrum (blue dashed). We elaborate on this point in the next section, as well as investigate alternate 
...
as solid green. In the revised PAPER-64 analysis in this paper, we choose to use a regularization scheme of $\widehat{\textbf{C}}_{\rm eff} \equiv 0.035 \, $Tr($\widehat{\textbf{C}})\textbf{I} + \widehat{\textbf{C}}$ ($\gamma = 0.035$) as a simple example of regularization that minimizes loss.}
...
their best (i.e. when $\sim3$$-4$ eigenmodes are down-weighted in the case of the black solid curve). However, for the remainder of this paper, we choose to use the weighting option of $\widehat{\textbf{C}} \equiv \widehat{\textbf{C}} + 0.035 \,$Tr($\widehat{\textbf{C}})\textbf{I}$, which we will denote as $\widehat{\textbf{C}}_{\rm eff}$. We choose this weighting scheme merely as a simple example of regularizing PAPER-64 covariances, though we note that in Kolopanis et al. (\textit{submitted}), we choose the most optimal regularization parameter for our final limits for every redshift bin independently. We also note that all of the regularizations described here perform similarly to the uniformly weighted case. However, we find that the regularization of empirical covariances does outperform the uniform weighted case for redshift bins in which there are stronger foregrounds, as presented in Kolopanis et al. (\textit{submitted}).
...
\begin{equation}
 ...
 \end{equation}
\noindent where $i$ indexes LST (\citealt{jacobs_et_al2015}). For PAPER-64, our revised estimate of $N_{\rm days}$ is $\sim34$ 
days.
...
\begin{equation}
N_{\rm bls} = \frac{N_{\rm bls\_total}}{N_{\rm gps}}\sqrt{N_{\rm gps}^{2}-N_{\rm gps}},
\end{equation}
\noindent where, as with the \citetalias{ali_et_al2015} analysis, $N_{\rm gps} = 5$. Each baseline group averages down linearly as the number of baselines 
entering the group ($N_{\rm bls\_total}/N_{\rm gps}$) and then as the square root of the number of cross-multiplied pairs ($\sqrt{N_{\rm gps}^{2} - 
N_{\rm gps}}$). For the revised PAPER-64 analysis with only one baseline separation type, this becomes $N_{\rm bls} \sim 46$ instead 
of $51$. 
...
\eqref{eq:sense}. All three show good agreement, validating our analytical thermal noise calculation. 
...
on power spectrum results is shown in \citetalias{ali_et_al2015}. Most notably, detections of bias exist at all $k$'s without crosstalk 
...
Here we perform three hybrid jackknife/null tests on PAPER-64 data that aim to isolate systematics in the data and verify 
that they are not attributable to EoR. Similar to in Section \ref{sec:JackknifeOverview}, we take jackknives along different axes of the dataset to produce multiple power spectra. In addition, we difference them to tease out excess variances, which can be thought of as a type of null test.
...
a higher limit than \citetalias{ali_et_al2015} by a factor of $\sim$$10$ in mK. The reasons for a previously under-estimated limit and 
