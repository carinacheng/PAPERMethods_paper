...
\end{equation}
and we wish to estimate $s$.  The contaminant has a similar structure for its covariance, and is assumed uncorrelated
\begin{equation}
\langle \sigma_i \rangle = 0; \; \langle \sigma^2_i \rangle = s; \; {\rm and}~\langle \sigma \sigma^T \rangle = s I_{N \times N} \equiv S
\end{equation}
$\langle \phi_i \rangle = 0$, 
$\langle \phi^2_i \rangle = g$, $\angle \phi \phi^T \rangle = g I_{N \times N} \equiv F$, 
...
\end{equation}
so in this case, {\it where the covariance structure of the contaminant is identical to the signal}, it is only possible to achieve an unbiased estimate by using some additional information to estimate the additive bias and subtract it.
...
\begin{eqnarray}
\langle \phi \rangle & = & 0 \\
\langle \phi^2 \rangle & = & g \\
 \langle \phi \phi^T \rangle & = & g m m^T \equiv G \\
 m^T m & = & 1
\end{eqnarray}
This is the case where each data point is an independent draw from a variable with a fixed variance $s$ we would like to estimate, and $m$ represents a mode which is correlated across many data points with correlation given by $m$, and amplitude in realization of $\phi$.
