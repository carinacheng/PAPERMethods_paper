...
$\hat{\textbf{p}}_{x}$ is the power spectrum of the data, $\hat{\textbf{p}}_{r}$ is the power spectrum of the
data plus injection, and $P_{\rm out}$ is the difference in power between the two.

In short, we can summarize our signal loss computation as the ratio of $P_{\rm out}/P_{\rm in}$, evaluated at the data level $\textbf{x}$ (though our actual detailed computation is described in the next section). We motivate the fact that we can evaluate the output-to-input power spectrum ratio at $\textbf{x}$ by the following reasoning.
...
Given this form, a suitable estimate for $\p_{\rm eor}$ would be $\phat / c_{\rm eor}$. Although such an estimate leaves an additive bias from foregrounds that must be mitigated by other methods (as is the case for any attempt to measure the $21\,\textrm{cm}$ power spectrum), it normalizes $\p_{\rm eor}$ back to its correct level such that there is no multiplicative bias. The most conceptually straightforward way to compute $c_{\rm eor}$ is to model the foregrounds and EoR signal via simulations, and then to form the quantity
\begin{equation}
\label{eq:Deriv1}
...
$P_{out,\alpha} = \textbf{e}^{\dagger}\textbf{R}_{r}\textbf{Q}^{\alpha}\textbf{R}_{r}\textbf{e}$. In short, this naive 
